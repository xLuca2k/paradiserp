...
function vRP.getUserBusiness(user_id, cbr)
  ...
  if user_id   then
    local rows = exports.oxmysql:executeSync("SELECT name,description,capital,laundered,reset_timestamp FROM vrp_user_business WHERE user_id = @user_id", {user_id = user_id})
    ...
    if business and os.time() >= business.reset_timestamp+cfg.transfer_reset_interval*60 then
      exports.oxmysql:query("UPDATE vrp_user_business SET laundered = 0, reset_timestamp = @time WHERE user_id = @user_id", {user_id = user_id, time = os.time()}, function()end)
...
function vRP.closeBusiness(user_id)
  exports.oxmysql:query("DELETE FROM vrp_user_business WHERE user_id = @user_id", {user_id = user_id}, function()end)
end
...
local function open_business_directory(player,page) -- open business directory with pagination system
  ...
  local rows = exports.oxmysql:executeSync("SELECT user_id,name,description,capital FROM vrp_user_business ORDER BY capital DESC LIMIT @b,@n", {b = page*10, n = 10})
  local count = 0
          ...
          vRP.prompt(player,lang.business.addcapital.prompt(),"",function(player,amount)
            ...
            if amount > 0 then
              if vRP.tryPayment(user_id,amount) then
                exports.oxmysql:query("UPDATE vrp_user_business SET capital = capital + @capital WHERE user_id = @user_id", {user_id = user_id, capital = amount}, function()end)
            ...
            vRP.prompt(player,lang.business.launder.prompt({launder_left}),""..launder_left,function(player,amount)
              ...
              if amount > 0 and amount <= launder_left then
                if vRP.tryGetInventoryItem(user_id,"dirty_money",amount,false) then
                  ...
                  exports.oxmysql:query("UPDATE vrp_user_business SET laundered = laundered + @laundered WHERE user_id = @user_id", {user_id = user_id, laundered = amount}, function()end)
              ...
              vRP.prompt(player,lang.business.open.prompt_capital({cfg.minimum_capital}),""..cfg.minimum_capital,function(player,capital)
                ...
                if capital >= cfg.minimum_capital then
                  if vRP.tryPayment(user_id,capital) then
                    exports.oxmysql:query("INSERT IGNORE INTO vrp_user_business(user_id,name,description,capital,laundered,reset_timestamp) VALUES(@user_id,@name,'',@capital,0,@time)", {user_id = user_id, name = name, capital = capital, time = os.time()}, function()end)
