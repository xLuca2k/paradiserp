            ...
            vRP.prompt(player,lang.cityhall.identity.prompt_age(),"",function(player,age)
              age = parseInt(age)
                    ...
                    if vRP.tryPayment(user_id,cfg.new_identity_cost) then
                      vRP.generatePhoneNumber(function(phone_number)
                        exports.oxmysql:query("UPDATE vrp_users SET firstName = @firstname, secondName = @name, age = @age, phone_number = @number WHERE id = @user_id", {
                          user_id = user_id, 
                          firstname = firstname, 
                          name = name, 
                          age = age,
...
local function build_client_cityhall(source) -- marker , blip pentru identitate / buletin
  local user_id = vRP.getUserId(source)
  if user_id   then
    local x,y,z = table.unpack(cfg.city_hall)

    vRPclient.addBlip(source,{x,y,z,cfg.blip[1],cfg.blip[2],lang.cityhall.title()})
    vRPclient.addMarkerSign(source,{21,x,y,z-1.45,0.50,0.60,0.60,0,255,204,100,150,1,false,0})
