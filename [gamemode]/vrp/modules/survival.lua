        ...
        if(bandaged[user_id] == nil)then
          if vRP.tryGetInventoryItem(user_id,"bandaj",1,true) then
            ...
            SetTimeout(5000, function()
              ...
              vRPclient.notify(player,{"Succes: ~w~Ti-ai aplicat un Bandaj si ti-ai revenit cu 15%!"})
              bandaged[user_id] = nil
            end)
          end
        else
          vRPclient.notify(player,{"Eroare: ~w~Ti-ai aplicat un bandaj deja!, asteapta 5 secunde"})
          return
        end
      else
        vRPclient.notify(player,{"Eroare: ~w~Esti lesinat doar Medicul te poate ajuta!"})
      end
    end)
  end
end
...
vRP.registerMenuBuilder("jucator", function(add, data)
  ...
  if user_id   then
    ...
    if vRP.isUserInFaction(user_id,"Politie") or vRP.isUserInFaction(user_id,"Smurd") or vRP.isUserInFaction(user_id,"Jandarmerie") then
