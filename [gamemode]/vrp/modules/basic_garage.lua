...
function findVehsIds()
	theLastID = 0
	exports.oxmysql:query("SELECT id FROM vrp_user_vehicles ORDER BY id DESC LIMIT 1", {}, function(pvehicles)
		if #pvehicles > 0 then
							...
							if vname then
								...
								if vehicle then
									...
									if #rows > 0 then -- has vehicle
										...
										exports.oxmysql:query("DELETE FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle", {user_id = user_id, vehicle = vname}, function()end)
			...
			for k,v in pairs(garages) do
				...
				if group then
					...
					local garage_enter = function(player,area)
						local user_id = vRP.getUserId(source)
						if user_id   and vRP.hasPermissions(user_id,gcfg.permissions or {}) and gcfg.faction == nil and gcfg.vip == nil then
							local menu = garage_menus[gtype]
							if menu then
								vRP.openMenu(player,menu)
							end
						elseif (gcfg.faction   or gcfg.faction ~= "") and gcfg.vip == nil then
						...
						elseif (gcfg.vip   or gcfg.vip ~= 0) then
		...
		if(string.len(vehicle_plate)<=8)then
			if(string.len(vehicle_plate)<=0)then
				...
				if(vehicle_plate:match("%W")) then
					...
					if(kr >= 2) then
						exports.oxmysql:query("UPDATE vrp_user_vehicles SET vehicle_plate = @vehicle_plate WHERE user_id = @user_id AND vehicle = @vehicle", {vehicle = vehicle, user_id = user_id, vehicle_plate = vehicle_plate}, function()end) 
			...
			if usrList ~= "" then
                ...
                if (tonumber(amount)) then
                  if tonumber(amount) > 0 then
					exports.oxmysql:query('SELECT id FROM vrp_user_vehicles WHERE user_id = @user_id AND vehicle = @vehicle',{['user_id'] = user_id,['vehicle'] = name},function(pvehicle)
                      if #pvehicle > 0 then
                            ...
                            if ok then
                              ...
                              if (tonumber(money) >= tonumber(amount)) then
                                ...
                                vRP.getUserIdentity(pID, function(identity)
									exports.oxmysql:query("UPDATE vrp_user_vehicles SET user_id = @user_id WHERE user_id = @oldUser AND vehicle = @vehicle",{['user_id'] = user_id,['oldUser'] = playerID,['vehicle'] = name},function(OpunPeMamaLuAntonyXInPula) end)
                                end)
...
function vRP.givePlayerSpecialVeh(user_id, vehName, type)
    ...
    vRP.getUserIdentity(user_id, function(identity)
        exports.oxmysql:query("INSERT IGNORE INTO vrp_user_vehicles(user_id,vehicle,vehicle_plate,veh_type) VALUES(@user_id,@vehicle,@vehicle_plate,@veh_type)", {user_id = user_id, vehicle = vehName, vehicle_plate = thePlate, veh_type = type}, function()end)
    end)
end
